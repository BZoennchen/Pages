...
To evaluate a line of code, we press ``shift + return``, and to evaluate a code block, we press ``cmd + return`` on the Mac and ``ctrl + return`` on Windows and Linux.
...
I am not a fan of this shortcut and if you work with **sclang** you will encounter additional ones.
It adds required knowledge to understand a program written in **sclang** and steepen the learning curve.
...
There are many other parts of the **sclang**, and if you want to get started, I encourage you to visit the tutorials of [Eli Fieldsteel](https://www.youtube.com/user/elifieldsteel) or look at the official documentation.
...
There are hundreds of different ``UGens``s, they basically spit out real numbers over time.
...
We can change all arguments we defined, for example,

```supercollider
~sineplay.set(\freq, 500);
```

sets the frequency to 500 cycles per second which increases the pitch.
...
Since we add two identical sine waves together, we should hear the same sound but twice as loud.
...
In general, achieving more interesting sounds through modulation of different arguments is a good practice.
...
Let's plot the sound with and without being multiplied by the envelope:
...
To get a feeling for how a sound changes using different arguments, I recommend using ``play()`` in combination with ``MouseX`` and ``MouseY``.
The latter two ``UGen``s spit out values that depend on the position of your mouse cursor.
